...
#Recursively travel in a direction
    ...
    def play_move(self, col, row, symbol):
        ...
        #This checks the rows and columns to see
        #if there are any remaining valid moves to be
        #made. It invokes the already existing 
        #is_legal_move function to check if the move being attempted
        #is valid or not
        #this is pretty similar to count score in terms of
        #how you parse through both rows and columns
        #however it yields instead of returning score.
        #yield returns a generator that which spawns 
        #more successors off of it.
    def has_successor_moves(self, symbol):
        for rows in range (0, self.rows):
            for columns in range (0, self.cols):
                if self.is_legal_move(columns, rows, symbol):
                    yield (columns, rows)
